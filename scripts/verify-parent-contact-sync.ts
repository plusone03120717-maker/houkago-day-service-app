...
async function main() {
  ...
  try {
    ...
    console.log('1. 放デイの利用連絡を承認する')
    {
      ...
      // お迎えがある日の利用開始＝事業所到着＝学校到着（希望時刻）の10分後
      check('利用開始は事業所に着いた時刻になる', att?.service_start_time?.startsWith('10:10'), att?.service_start_time)
    ...
    console.log('\n11. 同じ日に放デイと日中一時の両方を割り振る')
    {
      ...
      // その日いちばん早い開始（＝日中一時）だけが、事業所到着の時刻にずれる
      check('日中一時は事業所到着の時刻から始まる', att?.daytime_support_start_time?.startsWith('09:10'), att?.daytime_support_start_time)
