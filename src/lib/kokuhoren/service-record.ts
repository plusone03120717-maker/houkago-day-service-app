...
import { buildFile, dayCode, decimalCode, num, toShiftJis } from './format'
...
export type ServiceRecordDay = {
  ...
  /** 専門的支援実施加算を算定する日（月の回数から利用日の早い順に割り当てる） */
...
export type ServiceRecordChild = {
  ...
  /** 様式種別番号。同じ事業所番号でも児発(0301)と放デイ(0501)で分かれる */
...
/**
 * 時間数 → 整数部＋小数部2桁のゼロ埋め表現。
 * 明細情報の算定時間数は整数2桁（2.5時間 → "0250"）、
 * 基本情報の算定時間数計は整数3桁（12.5時間 → "01250"）。
 */
export function hoursCode(hours: number, intDigits: number): string {
  return decimalCode(hours, intDigits, 2)
}
...
export function buildServiceRecordCsv(
  ...
  for (const c of children) {
    ...
    basic[18] = hoursCode(totalHours, 3)
    ...
    basic[33] = num(transportCount)

    // 161 専門的支援実施加算（回）
    basic[160] = num(c.days.filter((d) => d.specializedSupport).length)

    // 170 延長支援加算（回）
    basic[169] = num(c.days.filter((d) => d.extensionLevel > 0).length)

    // 算定していない加算の回数欄は空欄ではなく 0 を送る（取込成功済みの実データに合わせる）。
    // 45 は児童発達支援のみに存在する欄で、放課後等デイサービスでは空欄のまま。
    for (const i of [34, 35, 116, 120, 158, 161, 162, 166, 171]) basic[i] = '0'
    if (form === '0301') basic[44] = '0'
    ...
    for (const d of [...c.days].sort((a, b) => a.date.localeCompare(b.date))) {
      ...
      if (d.specializedSupport) detail[100] = '1'             // 101 専門的支援加算（支援実施時）
