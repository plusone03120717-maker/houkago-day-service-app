...
export default async function EditCertificatePage({
  ...
  const [{ data: child }, { data: cert }] = await Promise.all([
    ...
    supabase
      ...
      .select('id, certificate_number, service_type, start_date, end_date, max_days_per_month, copay_limit, copay_category, municipality, upper_limit_manager, decision_service_code, contract_amount, contract_start_date, contract_end_date, contract_line_number, service_start_date, copay_exempt, upper_limit_manager_number, is_upper_limit_manager')
      .eq('id', certId)
      .eq('child_id', id)
      .single(),
  ])
    ...
    <div className="space-y-5">
      ...
      <CertificateForm
        ...
        initial={{
          ...
          service_start_date: cert.service_start_date ?? undefined,
          copay_exempt: cert.copay_exempt ?? false,
          upper_limit_manager_number: cert.upper_limit_manager_number ?? undefined,
          is_upper_limit_manager: cert.is_upper_limit_manager ?? false,
